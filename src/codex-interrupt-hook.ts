...
function locateCodexInterruptHook(
  ...
  const normalizedGroup = (value: unknown): unknown => {
    if (!value || typeof value !== "object" || Array.isArray(value)) return value;
    const group = structuredClone(value) as { hooks?: unknown[] };
    if (!Array.isArray(group.hooks) || group.hooks.length !== 1) return group;
    const hook = group.hooks[0];
    if (!hook || typeof hook !== "object" || Array.isArray(hook)) return group;
    const command = { ...(hook as Record<string, unknown>) };
    if (command.enabled === true || (command.enabled === false && options.allowNativeDisabled === true)) {
  ...
  if (!Array.isArray(groups) || !equal(normalizedGroup(groups[installed.groupIndex]), expectedGroup)) {
    if (Array.isArray(groups) && groups.some(group => equal(normalizedGroup(group), expectedGroup))) {
