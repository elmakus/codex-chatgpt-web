...
test("normalizes the live OpenAI-compatible CLIProxyAPI Muse catalog", async () => {
  ...
  const response = await modelsRequest(
    ...
    async () => Response.json({
      object: "list",
      data: [
        { id: "muse-spark-1.2", object: "model", created: 1, owned_by: "muse" },
  ...
  expect(body.models.find(model => model.slug === "muse-spark-1.3")).toMatchObject({
    display_name: "muse-spark-1.3",
    visibility: "list",
    supported_in_api: true,
    priority: 99,
  ...
  expect(body.models.some(model => model.slug === "claude-sonnet")).toBe(false);
});
...
test("keeps the primary catalog available when the Muse catalog schema is invalid", async () => {
  const config = defaultConfig("browser-only");
  const response = await modelsRequest(
    new Request("http://127.0.0.1:17841/v1/models", {
      headers: { authorization: "Bearer codex-oauth-token" },
    ...
    undefined,
    undefined,
    async () => Response.json({ object: "list" }),
  );
